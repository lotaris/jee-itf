...
import com.lotaris.j2ee.itf.model.TestGroupDefinition.SetupMethod;
...
/**
 * In order to run the integration test, a general test controller
 * is required to manage the injected session beans.
 * 
 * The abstract controller centralized the integration test management
 * to run every tests. The transaction management is also done to allow
 * commits and rollbacks for the tests.
 * 
 * A best practice is to keep test independent from the others. To enforce
 * that best practice, the test groups and the tests in the groups are shuffled.
 * Therefore, the order of test run are not the same between two different run.
 * It means that you cannot write tests that depends on one another test because
 * you cannot be sure that the dependency order is respected between runs.
 * 
...
public abstract class AbstractTestController implements TestController {
	...
	public Long run(Map<String, Filter> filters, Map<String, Listener> listeners, Long seed) {
		...
		Long internalSeed;
		if (seed == null || seed < 0) {
			internalSeed = new Long(System.currentTimeMillis());
		}
		else {
			internalSeed = new Long(seed);
		}
	...
	/**
	 * Check that the current test controller is well configured and can
	 * run the integration test in correct conditions. Try to fix missing 
	 * elements.
	 * 
	...
	/**
	 * Retrieve the EJB representation for a field discovered in the class
	 * that extends the current one.
	 * 
	 * WARNING: No check is done to know if the field is an EJB or not, it is 
	 *          expected to be already checked before calling this method.
	 * 
	...
	/**
	 * Run the setup methods for all test in the test group in separate transaction
	 * than the one used for the test method itself.
	 * @param testGroupDefinition The test group definition
	 */
	...
	/**
	 * Run the setup methods for a specific test in a the test group in a separate transaction
	 * than the one used for the test method itself.
	 * @param testGroupDefinition The test group definition
	...
	/**
	 * Run the setup methods for all test in the test group in the same transaction
	 * than the one used for the test method itself.
	 * @param testGroupDefinition The test group definition
	 */
	...
	/**
	 * Run the setup methods for a specific test in a the test group in the same transaction
	 * than the one used for the test method itself.
	 * @param testGroupDefinition The test group definition
	 * @param description The description that contains the test
	 */
	 ...
	 * Run a test method
	 * @param testGroupDefinition The test group definition
	 * @param tmc The test group configuration to get meta data (setup methods around the test method)
	...
	private void runTest(TestGroupDefinition testGroupDefinition, Description description) {
		...
		try {
			...
			StringBuilder sb = new StringBuilder("Invocation Target Exception:");
			sb.append("Message[").append(e.getCause().getMessage()).append("]\n").append(writer.toString());

			description.fail(sb.toString());
